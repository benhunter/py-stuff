/*
  ...
  (Based on: Mediawiki 1.18.1 hash function for seed recovery attack. Author: George Argyros (argyros.george@gmail.com))
 */
...
hexConvert(char buf[], unsigned int n1)
{
...
char *
captcha1hash(unsigned int seed, char hash[])
{
//  MD5_CTX md5;
  uint32_t r1, r2;
  uint32_t state[N];
  ...
  r1 = temper(twist(p[M+OFFSET], p[0+OFFSET], p[1+OFFSET]));
  printf("%u\n", r1);
...
{
  int i;
  char buf[MD5_LEN]; // TODO fix buf length

  captcha1hash(atoi(argv[1]), buf);
